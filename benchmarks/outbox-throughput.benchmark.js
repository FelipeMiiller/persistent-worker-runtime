...
async function runBenchmark() {
  ...
  console.log(`  -> Main Thread Ingestion Time (non-blocking): ${dispatchIngestionTime.toFixed(2)}ms`);
  console.log(`  -> Ingestion Rate: ${(count / (dispatchIngestionTime / 1000)).toFixed(0)} tasks dispatched/second`);
  ...
  console.log(`  -> All ${count} tasks processed and confirmed in: ${totalProcessTime.toFixed(2)}ms`);
  console.log(`  -> Worker Processing Throughput: ${(count / (totalProcessTime / 1000)).toFixed(0)} jobs/sec\n`);

  await runtime.shutdown();

  console.log('=====================================================================');
  console.log('CONCLUSION:');
  console.log(`- Main Thread in-memory dispatch rate exceeds ${(count / (dispatchIngestionTime / 1000)).toFixed(0)} jobs/sec.`);
  console.log('- Enables instant HTTP response times while workers process outbox tasks off-thread.');
  console.log('=====================================================================');
}
